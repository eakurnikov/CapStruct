...
public class SmoothTwoParallelPlanes extends SmoothSurface implements CapillarSystem {
    ...
    protected Vector getParticleSpeedRotationAxisOfUpperPlane(final CartesianPoint point, final Vector normal) {
    ...
    protected CartesianPoint getHitPointWithUpperPlane(final NeutralParticle p) {
        double x = p.getCoordinate().getX();
        double y = p.getCoordinate().getY();
        double z = p.getCoordinate().getZ();
